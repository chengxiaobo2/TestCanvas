...
canvas.restore()
```
对要画的图形，要执行一系列的复杂的变换，我们的思路是，想象先执行 canvas.drawRect(rect,paint) 再执行一系列的变换，而这一系列的变换，可以用矩阵表示。矩阵提供了了pre/post的方法，canvas的变换对应的是矩阵的pre方法，pre的意思是在当前的变换前先执行这个变换，所以说最先执行的要写在最下面。

还有一种思路是，想象成坐标系的变换。
<img src="pic/matrix_3.png" width=600>

个人的总结，多个线性变换不建议理解为坐标系的变换，因为计算起来，太复杂。<br>
简单的，单个线性变换，可以理解为 drawXXX图形的变换，也可以理解为坐标系的变换。
